...
type orderItemR struct {
	Item  *Product `boil:"Item" json:"Item" toml:"Item" yaml:"Item"`
...
func (r *orderItemR) GetItem() *Product {
	if r == nil {
		return nil
	}
	return r.Item
...
var (
	orderItemAllColumns            = []string{"id", "order_id", "item_id", "quantity"}
	orderItemColumnsWithoutDefault = []string{"order_id", "item_id", "quantity"}
...
func (o *OrderItem) Item(mods ...qm.QueryMod) productQuery {
...
func (orderItemL) LoadItem(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrderItem interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.ItemOrderItems = append(foreign.R.ItemOrderItems, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ItemID == foreign.ID {
				...
				foreign.R.ItemOrderItems = append(foreign.R.ItemOrderItems, local)
				break
			}
		}
	}
...
// Adds o to related.R.ItemOrderItems.
func (o *OrderItem) SetItem(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Product) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &productR{
			ItemOrderItems: OrderItemSlice{o},
		}
	} else {
		related.R.ItemOrderItems = append(related.R.ItemOrderItems, o)
	}
